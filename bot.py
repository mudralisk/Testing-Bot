...
@client.event
async def on_ready():
    await client.change_presence(status=discord.Status.idle, activity=discord.Game('메이플스토리하는척'))
    print('Bot is ready.')
...
async def _Union(ctx, level: int):
    ...
    if level >= 42 and level < 46:
        embed = discord.Embed(title="유니온 42~46레벨",description="제1군영", color = 0x00ff00)
        await client.send_message(message.channel,embed=embed)
